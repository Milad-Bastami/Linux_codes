...
Greatly reduce time in some operations. Sorting a 1.8m line bed file goes from 43 seconds with LC_ALL="" to 3.2 seconds with LC_ALL=C
...
A quick check to make sure all lines have the same number of columns:
    ...
    check-columns filenamme

A modified version that takes the field seperator as second argument:

        function check-columns-s(){
          awk 'BEGIN {FS="'$2'" }{ print NF }' $1 | sort -u
        }
        check-columns-s filename ","

Output tab-delimited/coma delimited output so that the columns are aligned
...
Do thing per chromosome

    export CHROM="$(seq 1 22) X Y MT" # define an envir variable
    for i in $CHROM;do *your code*;done # use it
